...
func BenchmarkLogitLogger(b *testing.B) {
	...
	// 测试用的日志记录器
	logger := logit.NewLogger(logit.DebugLevel, logit.NewDefaultHandlerWithoutEncoder(&nopWriter{}, timeFormat))
...
//    logger := logit.NewLogger(logit.DebugLevel, logit.NewDefaultHandlerWithoutEncoder(file, timeFormat))
//
//    // 测试用的日志任务
//    logTask := func() {
//        logger.Debug("debug...")
