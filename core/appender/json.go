...
// newJsonAppender returns a json appender.
...
func (ja jsonAppender) appendArray(dst []byte, key string, length int, fn func(innerDst []byte, index int) []byte) []byte {
	dst = ja.appendKey(dst, key)
...
func (ja jsonAppender) AppendBytes(dst []byte, key string, values []byte) []byte {
	return ja.appendArray(dst, key, len(values), func(innerDst []byte, index int) []byte {
		...
		innerDst = appendEscapedByte(innerDst, values[index])
		innerDst = append(innerDst, jsonStringQuotation)
		return innerDst
	})
}
...
func (ja jsonAppender) AppendRunes(dst []byte, key string, values []rune) []byte {
	return ja.appendArray(dst, key, len(values), func(innerDst []byte, index int) []byte {
		innerDst = append(innerDst, jsonStringQuotation)
		innerDst = appendEscapedRune(innerDst, values[index])
		innerDst = append(innerDst, jsonStringQuotation)
		return innerDst
	})
}
...
func (ja jsonAppender) AppendFloat32s(dst []byte, key string, values []float32) []byte {
	return ja.appendArray(dst, key, len(values), func(innerDst []byte, index int) []byte {
		...
		return strconv.AppendFloat(innerDst, value64, 'f', -1, 64)
	})
}
...
func (ja jsonAppender) AppendTimes(dst []byte, key string, values []time.Time, format string) []byte {
	return ja.appendArray(dst, key, len(values), func(innerDst []byte, index int) []byte {
		...
		innerDst = append(innerDst, jsonStringQuotation)
		innerDst = values[index].AppendFormat(innerDst, format)
		innerDst = append(innerDst, jsonStringQuotation)
		return innerDst
	})
}
...
func (ja jsonAppender) AppendErrors(dst []byte, key string, values []error) []byte {
	return ja.appendArray(dst, key, len(values), func(innerDst []byte, index int) []byte {
		if values[index] == nil {
			return append(innerDst, jsonNull...)
		}

		innerDst = append(innerDst, jsonStringQuotation)
		innerDst = appendEscapedString(innerDst, values[index].Error())
		innerDst = append(innerDst, jsonStringQuotation)
		return innerDst
	})
}
